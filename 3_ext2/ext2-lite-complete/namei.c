...
//                         struct inode *dir, struct dentry *dentry,
//                         const char *symname)
static int ext2_symlink(struct user_namespace *mnt_userns,
                        struct inode *dir, struct dentry *dentry,
                        const char *symname)
{
...
static int ext2_mknod(struct user_namespace *mnt_userns,
                      struct inode *dir, struct dentry *dentry,
                      umode_t mode, dev_t rdev)
{
	struct inode * inode;
...
out_dir:
	if (dir_de)
		...
		// folio_kunmap(dir_folio, dir_de);
		kunmap_local(dir_de);
out_old:
	// folio_release_kmap(old_folio, old_de);
	// folio_kunmap(old_folio, old_de);
	kunmap_local(old_de);
	return err;
}
