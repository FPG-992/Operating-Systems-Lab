...
static int __init init_ext2_fs(void)
{
	...
	err = register_filesystem(&ext2_fs_type); //registers the ext2-lite filesystem in the kernel by calling the register_filesystem() function
	//with the address of the ext2_fs_type structure as an argument

	if (err) {
		destroy_inodecache(); //destroys the inode cache by calling the destroy_inodecache() function for the ext2 filesystem 
		//because the filesystem is not registered in the kernel
	}
...
static void __exit exit_ext2_fs(void)
{
	...
	unregister_filesystem(&ext2_fs_type);	// unregisters the filesystem from the kernel by calling the unregister_filesystem() function 
	//with the address of the ext2_fs_type structure as an argument 

	destroy_inodecache(); //destroys the inode cache by calling the destroy_inodecache() function for the ext2 filesystem 
	//because the filesystem is no longer registered in the kernel
}

MODULE_AUTHOR("Filippos Giannakopoulos"); /* ? */
