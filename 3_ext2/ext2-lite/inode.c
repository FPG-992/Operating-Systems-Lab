                                         ...
                                         struct buffer_head **p)
{
	...
	*p = NULL;
	...
	/* ? */
	block_group = (ino - 1) / inodes_pg; // ino = inode number and we know that inodes_pg is the number of inodes
		
	/*get the block descriptor for the block_group*/
	gdp = ext2_get_group_desc(sb, block_group, &bh); // bh = buffer_head pointer of the block descriptor
	if (!gdp){ // if the block descriptor is not found
		goto eio; // return an error, specifically eio which means "I/O error"
	}

	/* Figure out the offset within the block group inode table */
	/* ? */
	block = le32_to_cpu(gdp->bg_inode_table); // we convert little endian to cpu endian
	
	unsigned long index = (ino - 1) % inodes_pg; //we know that inodes_pg is the number of inodes, 
	// we find the index of the inode in the inode table

	unsigned long inode_table_block = block + (index * inode_sz) / blocksize; // we know that block is a valid block number, inode_sz is the size of an inode,
	// and blocksize is the size of a block, now we know which block the inode is in

	//we calculate the offset of the inode in the block
	offset = (index * inode_sz) % blocksize; // inode_sz is the size of an inode, and blocksize is the size of a block

	/* Return the pointer to the appropriate ext2_inode */
	/* ? */

	bh = sb_bread(sb, inode_table_block); // we read the block that contains the inode
	if (!bh){ // if the block is not found
		goto eio; // return an error, specifically eio which means "I/O error"
		}

	*p = bh; // we return the buffer_head pointer of the block that contains the inode

	struct ext2_inode *inode = (struct ext2_inode *)(bh->b_data + offset); // we find the inode in the block by the inode table
	//with offset we find where it is exactly in the block 
	// we typecast the data of the block to an ext2_inode pointer to get the inode

	return inode; // we return the inode
