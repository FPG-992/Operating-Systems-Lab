...
ext2_dirent *ext2_find_entry(struct inode *dir, const struct qstr *child, struct folio **foliop){
	...
	for (i=0; i < npages; i++) {
		...
		/* Χρησιμοποιείται για την ανάγνωση
		σελίδων ενός καταλόγου, για αυτό και η πρώτη παράμετρος ονομάζεται
		dir (το struct folio είναι μία δομή που αναπαριστά είτε μία σελίδα είτε
		ένα σύνολο συνεχόμενων σελίδων μνήμης).*/

		//we check if we have an error getting the address of the i-th page of the directory
		if (IS_ERR(kaddr)){
			return ERR_CAST(kaddr); //return the error
		}

		//we check for the last byte of the i-th page of the directory (page_nr) so we know until what byte we should search
		unsigned last_byte = ext2_last_byte(dir, i);
		/*
		Επιστρέφει το τελευταίο χρησιμοποιούμενο byte της σελίδας page_nr του inode. Χρησιμοποιείται για σελίδες που περιέχουν directory entries του ext2 (ext2_dirent), ώστε να γνωρίζουμε μέχρι ποιο
		σημείο να ψάξουμε.
		*/

		for (unsigned offset = 0; offset < last_byte;) { //we iterate through the page
			de = (ext2_dirent *)(kaddr + offset); //we get the directory entry by adding the offset to the address of the page and casting it to ext2_dirent


			//we check if the index node number is valid and matches the name of the desired entry
			/*
			int ext2_match(int len, const char *const name, ext2_dirent
			*de): Συγκρίνει το de->name με το name και επιστρέφει 1 αν ταιριάζουν
			αλλιώς 0.*/
			if (de->inode && ext2_match(namelen, name, de)) {
				//entry was found and we return the directory entry
				return de; //directory entry
			}
			
			//we move to the next Dentry
			offset += le16_to_cpu(de->rec_len); //we move to the next directory entry by adding the length of the current directory entry to the offset
		}

		//we release the kmap of the folio Kmap is a kernel function that maps a page into the kernel's virtual address space
		folio_release_kmap(*foliop, kaddr);


	}
	return ERR_PTR(-ENOENT); //entry was not found in any of the pages of the directory
}
