                               ...
                               struct buffer_head *bitmap_bh, unsigned long *count)
{
    ...
    /* Find the first free block in the group's bitmap */
    ...
    if (first_free_bit >= nblocks) {
        ext2_debug("no free blocks in group %d\n", group);
        return -ENOSPC; // No space left in the group
    }

    /* Attempt to allocate up to *count blocks starting from first_free_bit */
    for (i = first_free_bit; i < nblocks && num < desired; i++) {
        if (ext2_set_bit_atomic(sb_bgl_lock(EXT2_SB(sb), group), i, bitmap_bh->b_data)) {
            num++;
        } else {
            break; // Stop if unable to set the bit (block already allocated)
        }
    }

    /* Update the count and return the offset if allocation was successful */
    if (num > 0) {
        *count = num;

        /* Mark the bitmap buffer as dirty since we've modified it */
        mark_buffer_dirty(bitmap_bh);

        /* If the filesystem is mounted with synchronous writes, sync the buffer */
        if (sb->s_flags & SB_SYNCHRONOUS)
            sync_dirty_buffer(bitmap_bh);
    ...
    return -ENOSPC; // Allocation failed due to no contiguous space
}
